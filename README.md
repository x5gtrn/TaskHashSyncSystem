...
| `detect_deleted_omnifocus_tasks.py` | **PRE-STEP (v2.8)**: Detect tasks deleted from OmniFocus; cascade-delete from GitHub/Vault/sync_state |
| `reverse_sync.py` | **STEP 2**: Sync completed tasks from OmniFocus back to GitHub/Vault; **STEP 2.2 (v2.7)**: Reflect GitHub Issue status changes to OmniFocus |
| `scan_omnifocus_inbox.py` | **STEP 3**: Scan ALL OmniFocus tasks for hashless items; route to Vault Daily Note or GitHub Issue; auto-renames OmniFocus tasks via JXA |
| `parse_omnifocus_dump.py` | Parse raw `dump_database` text output into `all_tasks_raw.json`; accepts optional `sync_state` for `added_date` backfill |
| `update_issue_body.py` | Atomic diff-based GitHub Issue body editor (REQUIRED for all Issue body edits); used by both reverse_sync.py and detect_deleted_omnifocus_tasks.py |
...
| `existing_issue_updates.json` | Output: changes detected in existing GitHub Issues (new_tasks, completion_changes, deleted_tasks); consumed by reverse_sync.py STEP 2.2 |
| `precheck_requests.json` | Existence checks Claude MUST run (get_task_by_id) before batch_add_items |
| `omnifocus_dump.txt` | Raw text output from `dump_database` MCP call; input to both detect_deleted_omnifocus_tasks.py and scan_omnifocus_inbox.py --dump-file |
| `all_tasks_raw.json` | Auto-generated by `scan_omnifocus_inbox.py --dump-file` (via parse_omnifocus_dump.py); also accepted directly via `--tasks` |
| `completed_tasks_raw.json` | Input to reverse_sync.py STEP 2.1 (Claude writes from filter_tasks MCP result) |
