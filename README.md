...
| `omnifocus_mcp.py` | **PHASE 2.1 (NEW)**: Unified MCP interface to OmniFocus via JXA; provides `batch_add_items()`, `get_task_by_name()`, `rename_task()`, `dump_database()` |

#### Phase 1: Emergency Cleanup
| File | Purpose |
|------|---------|
| `detect_phantom_tasks.py` | **PHASE 1.1 (NEW)**: Detect pending tasks that don't actually exist in OmniFocus; remove phantoms from sync_state.json |
| `emergency_hash_inbox_tasks.py` | **PHASE 1.2 (NEW)**: Find hashless tasks in OmniFocus Inbox, generate hashes, rename tasks, update sync_state.json |
...
| `sync_to_omnifocus_v2.py` | **PHASE 2.2 (NEW)**: Resolve parentTaskHash → parentTaskId; execute batch_add_items via omnifocus_mcp.py; capture returned OmniFocus IDs; update sync_state.json |
| `detect_deleted_omnifocus_tasks.py` | PRE-STEP: Detect tasks deleted from OmniFocus; cascade-delete from GitHub/Vault/sync_state |
...
| `tasks_to_sync.json` | Queue: output of prepare_sync.py, consumed by sync_to_omnifocus_v2.py |
| `existing_issue_updates.json` | Changes detected in existing GitHub Issues (new_tasks, completion_changes, deleted_tasks) |
| `precheck_requests.json` | Existence checks Claude MUST run before batch_add_items |
| `omnifocus_dump.txt` | Raw database dump from OmniFocus |
| `completed_tasks_raw.json` | Input to reverse_sync.py (completed tasks from OmniFocus) |
| `repositories.json` | Configuration: list of GitHub repositories to sync |

#### Reports & Logs
| File | Purpose |
|------|---------|
| `phantom_detection_report.json` | Output of detect_phantom_tasks.py |
| `emergency_hash_report.json` | Output of emergency_hash_inbox_tasks.py |
| `sync_to_omnifocus_report.json` | Output of sync_to_omnifocus_v2.py |
...
### Workflow D.2: Sync GitHub Issue Status Changes to OmniFocus (v2.7 - STEP 2.2)

**Goal:** Reflect GitHub Issue task completion status back to OmniFocus

**Scenario:** User marks 2 tasks complete in GitHub Issue #2, but OmniFocus still shows them open:
- "Task A (35efa56a)" marked [x] in GitHub
- "Task B (62d90568)" marked [x] in GitHub
- OmniFocus Project "Project X" still shows both as open

**Steps:**

1. **User updates GitHub Issue status**
   ```
   GitHub Issue #2: Project X
   - [x] Task A (35efa56a)       ← MARKED COMPLETE
   - [x] Task B (62d90568)       ← MARKED COMPLETE
   - [ ] Task C (7f1603d8)       ← Still open
   ```

2. **Run prepare_sync.py** (automatically via hook)
   - STEP 0.5 detects changes in existing GitHub Issues
   - Compares current GitHub Issue tasks with sync_state.json
   - Identifies completion_changes:
     ```json
     "completion_changes": [
       {
         "hash": "35efa56a",
     ...
     ```
   - Outputs `existing_issue_updates.json`

3. **STEP 2.2: Run reverse_sync.py with existing issue updates**
   ```bash
   python3 reverse_sync.py --existing-issue-updates existing_issue_updates.json
   ```

4. **Processing**
   - Loads completion_changes from existing_issue_updates.json
   - For each completion change:
     - Finds task in OmniFocus by name + hash
     - Updates task completion status via JXA:
       - `task.completed = true` for GitHub-completed tasks
   - Updates sync_state.json with new status:
     ```json
     {
       "35efa56a": {
         "status": "completed",
         "completed_at": "2026-05-30T14:32:00.000000"
       },
       "62d90568": {
         "status": "completed",
         "completed_at": "2026-05-30T14:32:00.000000"
...
5. **Result in OmniFocus**
   ```
   PROJECT: Project X
   ├── Task A (35efa56a) ✅        ← NOW MARKED COMPLETE
   ├── Task B (62d90568) ✅        ← NOW MARKED COMPLETE
   └── Task C (7f1603d8)           ← Still open
   ```

6. **Why STEP 2.2?**
   - Complements STEP 2.1 (OmniFocus → GitHub)
   - STEP 2.1: Reflects OmniFocus completions to GitHub
   - STEP 2.2: Reflects GitHub completions to OmniFocus
   - Ensures bidirectional sync of task status

**Key Feature:**
- ✅ Detects completion status changes made in GitHub
- ✅ Automatically updates OmniFocus via JXA
- ✅ Maintains sync_state.json with completion timestamps
- ✅ Prevents status desynchronization between systems

### Workflow D.5: Detect and Cascade Delete OmniFocus Deleted Tasks (v2.8)

**Goal:** Remove orphaned tasks from all locations when deleted from OmniFocus (PRE-STEP)

**Scenario:** User deletes 2 tasks in OmniFocus that were previously synced:
- "Finish report (a1f4c892)" was synced from Vault Daily Note
- "Review design (b2d8e641)" was synced from GitHub Issue #3

**Steps:**

1. **User deletes tasks in OmniFocus**
   ```
   OmniFocus INBOX:
   - ✅ Finish report (a1f4c892) [DELETED]
   - (no longer exists)

   OmniFocus PROJECT (App Dev):
   - ✅ Review design (b2d8e641) [DELETED]
   - (no longer exists)
   ```

2. **Run prepare_sync.py** (automatically via hook)
   - Prepares new tasks for sync
   - Generates `omnifocus_dump.txt` (will be saved by Claude)

3. **PRE-STEP: Run detect_deleted_omnifocus_tasks.py**
   ```bash
   python3 detect_deleted_omnifocus_tasks.py --dump-file omnifocus_dump.txt
   ```

4. **Detection Process**
   - Parses omnifocus_dump.txt: finds 20 tasks in OmniFocus
   - Checks sync_state.json: 22 tasks previously synced
   - Identifies 2 tasks not found in OmniFocus:
     - `a1f4c892` (Finish report) — source: Vault
     - `b2d8e641` (Review design) — source: GitHub Issue #3

5. **Cascade Deletion**
   ```
   ✓ Vault file: Task (a1f4c892) deleted from Calendar/Daily/2026/05/2026-05-15.md
   ✓ GitHub Issue #3: Task (b2d8e641) deleted
   ✓ Removed 2 task(s) from sync_state.json
   ```

6. **Results After PRE-STEP**
   - Vault Daily Note 2026-05-15 updated:
     ```markdown
     # Task deleted here
     (other tasks remain unchanged)
     ```
   - GitHub Issue #3 updated:
     ```
     - [x] Old Task 1 (hash1)
     - [ ] Old Task 2 (hash2)
     # Tasks b2d8e641 removed from here
     ```
   - sync_state.json cleaned:
     ```json
     {
       // a1f4c892 removed
       // b2d8e641 removed
...
7. **Why PRE-STEP?**
   - Runs BEFORE forward sync (STEP 1)
   - Ensures clean sync_state.json baseline
   - Prevents hash collisions with newly added tasks
   - Implements "State Integrity First" principle

**Key Benefits:**
- ✅ No orphaned tasks in GitHub or Vault
- ✅ sync_state.json stays accurate
- ✅ Supports `--dry-run` for safe preview
- ✅ Fully automated cascade across all locations

### Workflow E: All-Tasks Scan (OmniFocus → Vault Daily Note / GitHub)

**Goal:** Detect every TaskHash-less task across ALL OmniFocus tasks and route each to the
correct destination (Vault Daily Note or GitHub Issue).

**Why this workflow?**
Users sometimes add tasks directly to OmniFocus (Inbox or a native Project like "Later")
without a TaskHash. These tasks must be discovered and routed before they become orphans.

**Steps:**

1. **Trigger** (runs as STEP 3 of the manual sync workflow)

2. **Claude calls `dump_database()` and saves the raw text to `omnifocus_dump.txt`**
   (a single Write call — no manual JSON construction required)

3. **Run scan_omnifocus_inbox.py with `--dump-file`**
   ```bash
   python3 scan_omnifocus_inbox.py --dump-file omnifocus_dump.txt
   ```
   Internally, the script calls `parse_omnifocus_dump.py` to convert the raw text into
   `all_tasks_raw.json`, then proceeds with classification and routing.

4. **Classification output**
   ```
   ✓ Parsed 23 tasks from dump file → all_tasks_raw.json
   Loaded 23 tasks from OmniFocus (all projects + inbox)
...
6. **GitHub Issue #2 body updated**:
   ```
   - [ ] Write report (b9c0d1e2)
   ```

7. **OmniFocus task names updated automatically (JXA)**:
   The script writes a temporary JXA script to `/tmp/_of_rename.js` and executes it via
   `osascript -l JavaScript` for each renamed task. No manual `edit_item` MCP calls needed.
   - `"Clean desk"` → `"Clean desk (a1f2b3c4)"`
   - `"Buy milk"` → `"Buy milk (d5e6f7a8)"`
   - `"Write report"` → `"Write report (b9c0d1e2)"`
   
   Results are logged in `inbox_rename_requests.json` (audit trail only).
...
### all_tasks_raw.json Schema

Input to `scan_omnifocus_inbox.py`. Claude creates this file by calling `dump_database()`
and normalizing all incomplete tasks from the result.
...
|-------|------|----------|-------------|
| `id` | string | Yes | OmniFocus internal task ID (used for `edit_item` rename) |
...
remove_hash(task_name) -> str
  ...
  # Also handles no-space form: "Design ad creative(a1f4c892)" → "Design ad creative"

has_hash(task_name) -> bool
  # Check if task name contains hash
  # Pattern: (optional space) + "(XXXXXXXX)" at end of string
  # Accepts both "Task (abc12345)" and "Task(abc12345)"
  # The no-space form can appear when OmniFocus omits the leading space

extract_hash(task_name) -> str | None
  # Extract hash from task name (returns 8-char hex, no parentheses)
...
### parse_omnifocus_dump.py

Converts the raw text output from `dump_database` MCP call into the structured
`all_tasks_raw.json` format consumed by `scan_omnifocus_inbox.py`.
...
**Usage (as library — called internally by scan_omnifocus_inbox.py):**
```python
from parse_omnifocus_dump import parse_omnifocus_dump
result = parse_omnifocus_dump(dump_text, sync_state=state)
# result = {"tasks": [...]}
```

**Key parameters:**
- `dump_text` — raw string output from `dump_database`
- `sync_state` — optional dict loaded from `sync_state.json`; when provided, backfills
  `added_date` for already-tracked tasks by reading `synced_at` from their state entry.
  This ensures tasks re-encountered in later sync runs are routed to their original Daily
  Note rather than always falling back to today.
...
**Parsing rules:**
- Lines starting with `F:` = Folder (context only; not emitted as tasks)
- Lines starting with `P:` = Project (context for child tasks)
- Lines starting with `•` = Task; indentation (3 spaces per level) determines hierarchy
- Due dates parsed from `[DUE:M/D]` suffix
- Container tasks (first child with same name as parent Project) are skipped
- Duplicate `(project, task_name, indent)` tuples are deduplicated

### scan_omnifocus_inbox.py

Scans ALL OmniFocus tasks (Inbox + every Project) for TaskHash-less tasks and routes each
to the correct destination based on parent Project identity.

**Usage:**
```bash
# Recommended: pass raw dump_database output directly (no manual JSON needed)
python3 scan_omnifocus_inbox.py --dump-file omnifocus_dump.txt [--date YYYY-MM-DD] [--dry-run] [--verbose]

# Alternative: supply pre-built all_tasks_raw.json
python3 scan_omnifocus_inbox.py --tasks all_tasks_raw.json [--date YYYY-MM-DD] [--dry-run] [--verbose]
```

`--dump-file` accepts the raw text output from `dump_database` and internally calls
`parse_omnifocus_dump.py` to produce `all_tasks_raw.json`. This eliminates the need for
Claude to manually construct the JSON file.
...
**Input format (`all_tasks_raw.json`) — also auto-generated by `--dump-file`:**
```json
{
...
8. Update sync_state.json
9. **Auto-rename OmniFocus tasks via JXA** — writes `/tmp/_of_rename.js` and calls `osascript -l JavaScript`; no manual `edit_item` MCP calls required
10. Write `inbox_rename_requests.json` (audit log of rename results)
11. Output `github_issue_additions.json` for Claude to add tasks to GitHub Issues

**Output files:**
- `inbox_rename_requests.json` — Audit log of JXA rename attempts (OK / NOT_FOUND per task)
- `github_issue_additions.json` — Tasks to add to GitHub Issue bodies (grouped by issue number)

### detect_deleted_omnifocus_tasks.py (v2.8 - PRE-STEP)

Detects tasks that exist in `sync_state.json` but have been deleted from OmniFocus,
then cascade-deletes them from GitHub Issues and Vault Daily Notes.

**Purpose:**
This PRE-STEP ensures a clean sync state by removing orphaned tasks before forward sync begins.
Implements the "State Integrity First" principle: clean deletions before adding new tasks.
...
**Process:**
1. Parse `omnifocus_dump.txt` (raw `dump_database` output from OmniFocus MCP)
2. Extract all task names currently in OmniFocus
3. For each task in `sync_state.json`:
   - Check if task exists in OmniFocus
   - If NOT found and has TaskHash: mark for deletion
4. **Cascade delete** deleted tasks from all locations:
   - **GitHub Issue**: Call `update_issue_body.py --remove-task <hash>`
   - **Vault Daily Notes**: Remove task line via regex pattern matching
   - **sync_state.json**: Remove task entry entirely
5. Report deletion summary

**Output:**
```
================================================================================
DETECTING OMNIFOCUS DELETIONS
================================================================================

✓ Found 42 tasks in OmniFocus
⚠️  Found 2 task(s) that were deleted from OmniFocus:

  • Task A (a1f4c892)
    Source: vault:Calendar/Daily/2026/05/2026-05-20.md:Task A
  • Task B (b2d8e641)
    Source: github:x5gtrn/LIFE#3:Task B

------------------------------------------------------------------------
CASCADE DELETION
------------------------------------------------------------------------

  ✓ Vault file: Task (a1f4c892) deleted from Calendar/Daily/2026/05/2026-05-20.md
  ✓ GitHub Issue #3: Task (b2d8e641) deleted
...
- `--verbose`: Show detailed deletion information for each task

**Execution Order:**
- **Must run BEFORE all other sync steps** (PRE-STEP)
- Ensures `sync_state.json` is clean baseline before forward sync
- Prevents hash collisions and orphaned data
...
**Usage:**
```bash
python3 reverse_sync.py --completed-tasks <file> [--existing-issue-updates <file>]
```

**STEP 2.1: Completion Reflection**
```bash
python3 reverse_sync.py --completed-tasks completed_tasks_raw.json
```
...
**STEP 2.2: GitHub Issue Status Changes (v2.7)**
```bash
python3 reverse_sync.py --existing-issue-updates existing_issue_updates.json
```

**Process (v2.7 addition):**
1. Load `existing_issue_updates.json` (output of `prepare_sync.py` STEP 0.5)
2. For each task with `completion_changes`:
   - Identify the task in OmniFocus
   - Update task completion status via JXA script:
     - GitHub Issue task marked [x] in GitHub → Mark complete in OmniFocus
     - GitHub Issue task marked [ ] in GitHub → Mark incomplete in OmniFocus
3. Update sync_state.json with new status
4. Report completion status changes

**Why STEP 2.2?**
- `prepare_sync.py` STEP 0.5 detects GitHub Issue status changes (new_tasks, completed_tasks)
- Some tasks may have been marked complete in GitHub but remain open in OmniFocus
- STEP 2.2 reflects GitHub status back to OmniFocus, ensuring bidirectional sync
- Complements STEP 2.1 (OmniFocus → GitHub)
...
- [x] **`--dump-file` option for scan_omnifocus_inbox.py** (May 11, 2026)
  - Accepts raw `dump_database` text output directly — no manual JSON construction
  - Internally calls `parse_omnifocus_dump.py` and writes `all_tasks_raw.json` automatically
  - Reduces STEP 3 of the sync workflow from multiple manual steps to a single command
- [x] **Automatic JXA task rename in scan_omnifocus_inbox.py** (May 11, 2026)
  - After routing, the script automatically appends TaskHash to OmniFocus task names
  - Uses JXA (`osascript -l JavaScript`) via a temporary `/tmp/_of_rename.js` file
  - Handles task names containing Japanese and special characters (encoding-safe)
  - Eliminates the need for manual `edit_item` MCP calls after STEP 3
- [x] **`parse_omnifocus_dump.py` sync_state integration** (May 11, 2026)
  - `parse_omnifocus_dump()` now accepts an optional `sync_state` dict
  - Already-tracked tasks have their `added_date` backfilled from `synced_at` in sync_state
  - Ensures re-encountered tasks are routed to their original Daily Note
- [x] **Space-tolerant TaskHash detection in task_hash.py** (May 11, 2026)
  - `has_hash()`, `extract_hash()`, `remove_hash()` now use pattern `r' ?\([0-9a-f]{8}\)'`
  - Correctly recognises `Task(hash)` (no space) in addition to `Task (hash)` (with space)
  - Prevents false "hashless" classification when OmniFocus omits the leading space
- [x] **Dropped-task false-positive fix in prepare_sync.py** (May 11, 2026)
  - `detect_existing_issue_updates()` now skips entries with `status: "dropped"` when building the set of synced hashes
  - Previously, dropped tasks were treated as "deleted" from the Issue on every sync cycle
- [x] **GitHub Issue Status Desync Fix (v2.7 - May 30, 2026)**
  - **Problem**: When GitHub Issue status changes were made (tasks marked complete in GitHub), they were NOT reflected back to OmniFocus
  - **Root Cause**: `reverse_sync.py` only queried OmniFocus completed tasks; GitHub Issue status changes were unidirectional
  - **Solution**: Added STEP 2.2 (Sync GitHub Issue Status Changes to OmniFocus)
    - `reverse_sync.py` now accepts `--existing-issue-updates` parameter
    - Processes `existing_issue_updates.json` output from `prepare_sync.py` STEP 0.5
    - Updates OmniFocus task completion status via JXA for tasks marked complete in GitHub
    - Ensures bidirectional sync: OmniFocus ↔ GitHub Status
- [x] **OmniFocus Deletion Cascade (v2.8 - May 30, 2026)**
  - **Problem**: When tasks were deleted from OmniFocus, they remained in GitHub Issues and Vault Daily Notes, creating orphaned data
  - **Solution**: Created `detect_deleted_omnifocus_tasks.py` as PRE-STEP (runs before all other sync steps)
    - Scans `omnifocus_dump.txt` for all OmniFocus tasks
    - Identifies tasks in `sync_state.json` that NO LONGER exist in OmniFocus
    - **Cascade deletes** from all three locations:
      - GitHub Issue body (via `update_issue_body.py --remove-task`)
      - Vault Daily Notes (via regex line removal)
      - `sync_state.json` (removes task entry entirely)
    - Supports `--dry-run` and `--verbose` flags for safe operation
    - **Execution Order**: PRE-STEP runs BEFORE forward sync, ensuring clean state baseline
    - **Rationale**: State Integrity First principle — clean deletions before adding new tasks

---

## Implementation Status

### Current State (v2.9 - May 11, 2026)

#### ✅ Fully Working

| Flow                                 | Status     | Component                 | Trigger     |
| ------------------------------------ | ---------- | ------------------------- | ----------- |
| **PRE-STEP:** OmniFocus Deletion Cascade | ✅ Complete | `detect_deleted_omnifocus_tasks.py` | Manual sync (first step) |
...
| OmniFocus (completed) → Vault/GitHub | ✅ Complete | `reverse_sync.py` (STEP 2) | Manual sync |
| **STEP 2.2:** GitHub Issue Status → OmniFocus | ✅ Complete | `reverse_sync.py --existing-issue-updates` | Manual sync |
| OmniFocus All Tasks → Vault/GitHub   | ✅ Complete | `scan_omnifocus_inbox.py --dump-file` | Manual sync |
