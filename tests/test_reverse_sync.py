...
#
# SAMPLE_STATE models:
#   aaaaaaaa - open vault task
#   bbbbbbbb - open github task
#   cccccccc - already completed vault task
#   60c6d084 - a github_project (used to test project_has_task_hash)
...
SAMPLE_STATE = {
    ...
    "bbbbbbbb": {
        "source_id": "github:x5gtrn/LIFE#2:Submit application to Acme Corp",
        "of_task_id": "t002",
        "of_task_name": "Submit application to Acme Corp (bbbbbbbb)",
        "status": "open",
...
class TestFindCompletedTasksInState(unittest.TestCase):
    ...
    def test_new_dict_format_matches(self):
        ...
        completed = find_completed_tasks_in_state(
            SAMPLE_STATE,
            [{"hash": "aaaaaaaa", "name": "Buy coffee beans (aaaaaaaa)"}]
        )
        self.assertIn("aaaaaaaa", completed)
    ...
    def test_github_task_matches(self):
        completed = find_completed_tasks_in_state(
            SAMPLE_STATE,
            ["Submit application to Acme Corp (bbbbbbbb)"]
        )
    ...
    def test_multiple_completions(self):
        completed = find_completed_tasks_in_state(
            SAMPLE_STATE,
            ["Buy coffee beans (aaaaaaaa)", "Submit application to Acme Corp (bbbbbbbb)"]
        )
        self.assertIn("aaaaaaaa", completed)
        self.assertIn("bbbbbbbb", completed)

    def test_already_completed_task_still_returned(self):
        # Even if status=completed in state, we still return it (caller decides)
        completed = find_completed_tasks_in_state(
            SAMPLE_STATE,
    ...
    def test_malformed_string_without_hash_not_matched(self):
        # String without parenthesised hash at end -> no match
        completed = find_completed_tasks_in_state(
            SAMPLE_STATE,
            ["Buy coffee beans no hash"]
        )
        self.assertEqual(completed, {})
...
class TestUpdateVaultFileCheckbox(unittest.TestCase):
    ...
    def test_marks_unchecked_as_checked(self):
        p = self._write_temp("- [ ] Buy coffee beans (aaaaaaaa)\n")
        with patch.object(rs, 'VAULT_ROOT', p.parent):
            result = update_vault_file_checkbox(
                Path(p.name), "Buy coffee beans", dry_run=False
            )
    ...
    def test_already_checked_returns_true(self):
        p = self._write_temp("- [x] Buy coffee beans (aaaaaaaa)\n")
        with patch.object(rs, 'VAULT_ROOT', p.parent):
            result = update_vault_file_checkbox(
                Path(p.name), "Buy coffee beans", dry_run=False
            )
    ...
    def test_does_not_duplicate_existing_completion_date(self):
        p = self._write_temp("- [x] Buy coffee beans (aaaaaaaa) ✅ 2026-05-07\n")
        with patch.object(rs, 'VAULT_ROOT', p.parent):
            update_vault_file_checkbox(
                Path(p.name), "Buy coffee beans",
                completed_date="2026-05-08", dry_run=False
    ...
    def test_adds_due_date_before_completion_date(self):
        ...
        self.assertLess(due_pos, done_pos)  # 📅 must come before ✅

    def test_updates_existing_due_date(self):
        p = self._write_temp("- [ ] Pay monthly rent (aaaaaaaa) 📅 2026-05-05\n")
        with patch.object(rs, 'VAULT_ROOT', p.parent):
            update_vault_file_checkbox(
                Path(p.name), "Pay monthly rent",
                due_date="2026-06-01",
    ...
    def test_dry_run_does_not_modify_file(self):
        original = "- [ ] Buy coffee beans (aaaaaaaa)\n"
    ...
    def test_updates_indented_task(self):
        ...
        self.assertIn("- [ ] Parent task", updated)  # parent untouched

    # -- multi-word task names --

    def test_multi_word_task_name(self):
        p = self._write_temp("- [ ] Fold and store laundry (59bc1bb3)\n")
        with patch.object(rs, 'VAULT_ROOT', p.parent):
            result = update_vault_file_checkbox(
                Path(p.name), "Fold and store laundry",
                completed_date="2026-05-08", dry_run=False
            )
        updated = p.read_text(encoding='utf-8')
        self.assertTrue(result)
        self.assertIn("- [x] Fold and store laundry", updated)
        self.assertIn("✅ 2026-05-08", updated)
...
class TestProjectHasTaskHash(unittest.TestCase):
    ...
    def test_returns_true_for_project_type_in_state(self):
        state = {
            "60c6d084": {
                "task_type": "project",
                "of_task_name": "Job Search Q3 (60c6d084)",
        ...
        self.assertTrue(project_has_task_hash("Job Search Q3", state))
    ...
    def test_returns_false_for_non_project_task_type(self):
        ...
        self.assertFalse(project_has_task_hash("Buy coffee beans", state))
