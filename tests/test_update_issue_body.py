...
#
# ISSUE_JOBS_BODY: Job search project (Issue #2 analogue)
#   Tracks recruiter contacts and follow-up tasks.
#
# ISSUE_DEV_BODY: App development project (Issue #3 analogue)
#   Tracks feature delivery and bug fixes.

ISSUE_JOBS_BODY = """\
- [x] Reply to recruiter email (35efa56a)
- [x] TCS Japan casual interview proposal (62d90568)
- [x] Reply to Maresuke Yamada (70a0e1f4)
- [ ] Follow up on Rayzel referral (4375b980)
    - [ ] Review the submitted job description (3d8c2904)
- [ ] Teams call with Joana Inamori on the 13th at 17:00 (c329bc12)""".splitlines()

ISSUE_DEV_BODY = """\
- [x] Create feedback page (51a9ee37) ✅ 2026-05-06
- [ ] Bug fixes (251383dc)
    - [ ] Remove Before and After label text (7f1603d8)""".splitlines()
...
class TestFindLineIndex(unittest.TestCase):
    ...
    def test_finds_top_level_line(self):
        idx = find_line_index(ISSUE_JOBS_BODY, "35efa56a")
        self.assertEqual(idx, 0)

    def test_finds_nested_line(self):
        idx = find_line_index(ISSUE_JOBS_BODY, "3d8c2904")
        self.assertEqual(idx, 4)

    def test_returns_none_when_not_found(self):
        idx = find_line_index(ISSUE_JOBS_BODY, "00000000")
        self.assertIsNone(idx)

    def test_finds_line_in_dev_issue(self):
        idx = find_line_index(ISSUE_DEV_BODY, "7f1603d8")
...
class TestDetectIndent(unittest.TestCase):
    ...
    def test_root_level_parent_gives_4_space_child_indent(self):
        ...
        indent = detect_indent(ISSUE_JOBS_BODY, "4375b980")
        self.assertEqual(indent, "    ")

    def test_nested_parent_gives_8_space_child_indent(self):
        # "3d8c2904" is already at 4-space indent -> child should be 8
        indent = detect_indent(ISSUE_JOBS_BODY, "3d8c2904")
        self.assertEqual(indent, "        ")

    def test_hash_not_found_defaults_to_4(self):
        indent = detect_indent(ISSUE_JOBS_BODY, "00000000")
        self.assertEqual(indent, "    ")
...
class TestOpAddTask(unittest.TestCase):
    ...
    def test_appended_to_end(self):
        lines = ["- [ ] First task (11111111)", "- [ ] Second task (22222222)"]
        result = op_add_task(lines, "Third task (33333333)")
        self.assertEqual(len(result), 3)
        self.assertTrue(result[-1].endswith("Third task (33333333)"))
...
class TestOpAddChild(unittest.TestCase):
    ...
    def test_child_uses_correct_indent(self):
        ...
        result = op_add_child(lines, "Child task (22222222)", "Grandchild task (33333333)")
        # Grandchild should have 8-space indent
        self.assertTrue(result[2].startswith("        "))

    def test_parent_not_found_exits(self):
        lines = ["- [ ] Task (aaaaaaaa)"]
        with self.assertRaises(SystemExit):
            op_add_child(lines, "Nonexistent task (00000000)", "Child (bbbbbbbb)")
...
class TestOpCheckTask(unittest.TestCase):
    ...
    def test_preserves_other_lines(self):
        ...
        result = op_check_task(lines, "aaaaaaaa")
        self.assertIn("[x]", result[0])
        self.assertIn("[ ]", result[1])  # Task Beta untouched
...
class TestOpUncheckTask(unittest.TestCase):
    ...
    def test_already_unchecked_is_noop(self):
        lines = ["- [ ] Not done yet (bbbbbbbb)"]
        result = op_uncheck_task(lines, "bbbbbbbb")
        self.assertEqual(result[0], "- [ ] Not done yet (bbbbbbbb)")

    def test_preserves_other_lines(self):
        lines = [
            "- [x] Task Alpha (aaaaaaaa)",
            "- [x] Task Beta (bbbbbbbb)",
        ]
        result = op_uncheck_task(lines, "aaaaaaaa")
        self.assertIn("[ ]", result[0])
        self.assertIn("[x]", result[1])  # Task Beta untouched
...
class TestRoundTrip(unittest.TestCase):
    ...
    def test_add_then_remove_is_noop(self):
        original = ["- [ ] Existing task (aaaaaaaa)"]
        after_add = op_add_task(list(original), "Temporary task (bbbbbbbb)")
    ...
    def test_check_then_uncheck_restores_line(self):
        lines = ["- [ ] Schedule meeting (aaaaaaaa)"]
    ...
    def test_jobs_issue_check_call_then_uncheck(self):
        lines = list(ISSUE_JOBS_BODY)
